...
function safeRemoveDirSync(dir: string, retries = 10, delayMs = 30): void {
  for (let i = 0; i < retries; i++) {
    try {
      rmSync(dir, { recursive: true, force: true });
      return;
    } catch (error: unknown) {
      if ((error as { code?: string })?.code === "EBUSY" && i < retries - 1) {
...
describe("Postgres logging config", () => {
  test("selects the Postgres Kysely backend", async () => {
    ...
    try {
      ...
      safeRemoveDirSync(configPath.dir);
    }
  });
});
