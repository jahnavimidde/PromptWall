...
const opensshFooter = ["-----END", "OPENSSH PRIVATE KEY-----"].join(" ");
const opensshKey = `${opensshHeader}
...
const rsaFooter = ["-----END", "RSA PRIVATE KEY-----"].join(" ");
const rsaKey = `${rsaHeader}
...
const privateKey = `${pkHeader}
MIIEvQIBADANBgkqhkiG9w0BAQEFAASCBKcwggSjAgEAAoIBAQC4v5Q8v5Q8v5Q8v
5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v
5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v5Q8v
${pkFooter}`;

const encHeader = ["-----BEGIN", "ENCRYPTED PRIVATE KEY-----"].join(" ");
const encFooter = ["-----END", "ENCRYPTED PRIVATE KEY-----"].join(" ");
const encryptedKey = `${encHeader}
...
describe("detectSecrets", () => {
  ...
  test("avoids false positives - text with BEGIN but not full block", () => {
    const text = `This text contains ${opensshHeader} but not the full key`;
    const result = detectSecrets(text, defaultConfig);
    expect(result.detected).toBe(false);
    expect(result.matches).toHaveLength(0);
  });
  ...
  test("avoids false positives - just END marker", () => {
    const text = `Some text with ${opensshFooter} at the end`;
    const result = detectSecrets(text, defaultConfig);
    expect(result.detected).toBe(false);
    expect(result.matches).toHaveLength(0);
  });
...
const openaiApiKey = ["sk", "proj", "abc123def456ghi789jkl012mno345pqr678stu901vwx"].join("-");
const anthropicApiKey = [
  "sk",
  "ant",
  "api03",
  "abc123def456ghi789jkl012mno345pqr678stu901vwxyz0123456789abcdefghijklmnopqrstuvwxyz",
].join("-");
const stripeTestKey = ["sk", "test", "abc123def456ghi789jkl012"].join("_");
const stripeLiveKey = ["sk", "live", "xyz789abc123def456ghi789"].join("_");
const revenueCatKey = ["sk", "abcdefghijklmnopqrstuvwx"].join("_");
const awsAccessKey = ["AKIA", "IOSFODNN7EXAMPLE"].join("");
const githubToken = ["ghp", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx1234"].join("_");
const githubOAuthToken = ["gho", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx5678"].join("_");
const jwtToken = [
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9",
  "eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ",
  "SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c",
].join(".");
const bearerToken = `Bearer ${["eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9", "abcdefghijk"].join("")}`;
