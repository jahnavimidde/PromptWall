  ...
  test("successfully constructs a debug envelope from pipeline outputs", () => {
    const mockPrivacy = {
      ...
      secretsResult: {
        ...
        detection: {
          detected: true,
          matches: [{ type: "AWS_SECRET_ACCESS_KEY", start: 0, end: 10, value: ["AKIA", "IOSFOD"].join("") }],
        },
        maskingContext: {
      ...
      piiResult: {
        hasPII: true,
        detection: {
          allEntities: [{ entity_type: "EMAIL_ADDRESS", score: 0.99, start: 10, end: 27 } as any],
          scanTimeMs: 42,
        },
      },
      piiMaskingContext: {
    ...
    } as unknown as PrivacyPipelineResult<any>;

    const originalMessages = [
      { role: "user", content: "My AWS is " + ["AKIA", "IOSFOD"].join("") + " and email is sarah@example.com" }
    ];
    const maskedMessages = [
      { role: "user", content: "My AWS is [[AWS_SECRET_ACCESS_KEY_1]] and email is [[EMAIL_ADDRESS_1]]" }
    ];
    ...
    expect(result.debug.originalPrompt).toBe("My AWS is " + ["AKIA", "IOSFOD"].join("") + " and email is sarah@example.com");
    expect(result.debug.maskedPrompt).toBe("My AWS is [[AWS_SECRET_ACCESS_KEY_1]] and email is [[EMAIL_ADDRESS_1]]");
